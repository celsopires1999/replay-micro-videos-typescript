...
import ExistsError from "#seedwork/domain/errors/exists.error";
  ...
  implements RepositoryInterface<E>
{
  ...
  async insert(entity: E): Promise<void> {
    ...
    if (indexFound !== -1) {
      throw new ExistsError(`Entity exists already with id ${entity.id}`);
    }
  ...
  async update(entity: E): Promise<void> {
    ...
    if (indexFound === -1) {
      throw new NotFoundError(
        `Entity not found on update using id ${entity.id}`
      );
    }
  ...
  async delete(id: string | UniqueEntityId): Promise<void> {
    ...
    if (indexFound === -1) {
      throw new NotFoundError(`Entity not found on delete using id ${_id}`);
    }
